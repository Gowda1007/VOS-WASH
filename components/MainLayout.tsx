...
import { Icon, Logo, Button, Vari } from './Common';
...
export const MainLayout: React.FC<MainLayoutProps> = ({ children, currentView, pageTitle, onNavigate, onNewInvoice, onTakeOrder }) => {
    ...
    const SidebarContent = () => (
         <div className="flex flex-col h-full bg-white dark:bg-slate-800">
            <div className="flex flex-col items-center justify-center border-b border-slate-200 dark:border-slate-700 py-4">
                <Vari className="w-36 mb-2" />
                <div className="flex items-center space-x-2">
                    <Logo className="w-12 h-12 text-blue-700 dark:text-blue-400" />
                    <div className="flex flex-col items-end">
                        <h1 className="text-3xl font-bold text-blue-700 dark:text-blue-400 leading-none">{t('app-name')}</h1>
                        <p className="text-xs text-slate-500 dark:text-slate-400 leading-none text-right">{t('app-tagline')}</p>
                    </div>
                </div>
            </div>
