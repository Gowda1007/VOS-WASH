...
// This custom hook calculates and applies the correct scale transform to the invoice content.
        ...
        const observer = new ResizeObserver(entries => {
            for (const entry of entries) {
                ...
                if (wrapperWidth > 0) {
                    ...
                    // Set the wrapper's height to match the scaled content to maintain layout space
...
export const InvoicePreview: React.FC<{ invoiceData: Invoice }> = ({ invoiceData }) => {
  ...
  const BASE_INVOICE_WIDTH = 800; // Optimal fixed width for high-quality rendering
  ...
  return (
    <div id="invoice-preview-container" ref={wrapperRef} className="w-full relative">
       {/* 
        This is the A4 content that will be captured for PDF.
        It has a fixed width and is scaled down with CSS transform to fit its container,
        ensuring perfect proportional scaling of all content.
      */}
      <div 
        ...
        <div className="p-12 m-0 flex-grow flex flex-col">
            <div className="flex-grow">
              ...
              <header className="text-center mb-5 border-b pb-4">
                  ...
                  <div className="flex items-center justify-center">
                      ...
                      <div className="flex flex-col">
                          <h1 className="text-5xl font-bold text-blue-700">VOS WASH</h1>
                          <p className="text-xl text-right">(Clean Everything)</p>
                      </div>
                  </div>
              </header>
              ...
              <section className="flex justify-between items-start mb-4 text-lg">
                <div>
                  <h2 className="font-bold text-xl text-gray-700 mb-2">BILL TO:</h2>
                  <p>{customerName}</p>
                ...
                <div className="text-right">
                  ...
                  <p className="text-gray-500"><strong>Invoice #:</strong> {invoiceNumber}</p>
                  <p className="text-gray-500"><strong>Date:</strong> {invoiceDate}</p>
              ...
              <section className="mb-8">
                  <table className="w-full text-left border border-blue-600 rounded-lg overflow-hidden">
                      <thead className="bg-blue-600 text-white border-b-2 border-blue-800">
                          <tr>
                              <th className="p-1.5 font-semibold">Sl No.</th>
                              <th className="p-1.5 font-semibold">SERVICE</th>
                              <th className="p-1.5 font-semibold text-center">QTY</th>
                              <th className="p-1.5 font-semibold text-right">PRICE</th>
                              <th className="p-1.5 font-semibold text-right">TOTAL</th>
                          </tr>
                      </thead>
                      <tbody>
                          {services.map((s, i) => (
              ...
              <section className="flex justify-end mt-auto text-lg">
                  <div className="w-1/2 ">
                      ...
                      <div className="flex justify-between text-gray-700 mb-1"><p>GST (18%):</p><p>+ ₹{tax.toFixed(2)}</p></div>
                      <div className="flex justify-between text-gray-700 mb-1 border-b border-dashed "><p>Discount (Equal to GST):</p><p>- ₹{discount.toFixed(2)}</p></div>
                      
                      <div className={`flex justify-between text-base mt-1 pt-1 ${hasFinancialDetails ? 'border-b-2 border-dashed border-gray-400' : ''} font-extrabold text-blue-700`}>
                          <p>{hasFinancialDetails ? 'SERVICE TOTAL:' : 'TOTAL:'}</p>
                          <p>₹{serviceTotal.toFixed(2)}</p>
                      </div>
                      
                      {hasFinancialDetails && (
                          <div className="mt-1 pt-1 border-b-2 border-dashed border-gray-400 pb-2">
                      ...
                      <div className="flex justify-between font-extrabold text-xl mt-1 pt-1 border-t-4 border-blue-600 bg-blue-50 p-2 rounded-lg shadow-inner">
      ...
      <style>{`
        ...
        @media print {
            ...
            #invoice-preview-container {
                height: auto !important; /* Reset height for printing */
            }
            #invoice-preview-content {
                position: absolute;
                left: 0;
