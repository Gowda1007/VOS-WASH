...
export const CustomerListPage: React.FC<CustomerListPageProps> = ({ customers, invoices, onViewCustomer }) => {
    ...
    const handleExport = async () => {
        ...
        toast.success(t('export-pdf-success-message', 'Customer list exported to PDF.'));
    };
    ...
    return (
        <div className="space-y-6">
            <p className="text-slate-500 dark:text-slate-400">{t('you-have-unique-customers', 'You have {count} unique customers.').replace('{count}', customers.length.toString())}</p>
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 text-sm text-slate-600 dark:text-slate-400">
                                <tr>
                                    ...
                                    <th className="p-4 font-semibold">{t('contact', 'Contact')}</th>
                                    <th className="p-4 font-semibold text-center">{t('invoices')}</th>
                                    <th className="p-4 font-semibold text-right">{t('total-spent', 'Total Spent')}</th>
                                </tr>
                            </thead>
                            <tbody>
                                {hasSearchResults ? (
    ...
    <div onClick={() => onView(customer)} className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer active:bg-slate-100 dark:active:bg-slate-700/50">
        <div className="flex justify-between items-start">
            ...
            <div className="text-right">
                <p className="text-xs text-slate-500 dark:text-slate-400">{t('invoices')}</p>
        ...
        <div className="mt-4 pt-2 border-t border-slate-200 dark:border-slate-700 text-right">
            <p className="text-xs text-slate-500 dark:text-slate-400">{t('total-spent', 'Total Spent')}</p>
